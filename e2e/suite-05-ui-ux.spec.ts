...
test.describe('Test Suite 5: UI/UX', () => {
  ...
  test('TC-5.3: Loading States', async ({ appPage, page }) => {
    ...
    const loadingText = page.locator('text=Loading').or(
  ...
  test('TC-5.4: WASM Status Indicator', async ({ appPage, page }) => {
    ...
    const wasmBadge = page.locator('text=WASM Ready').or(
      page.locator('.bg-green-100, .bg-green-900\\/30').filter({ hasText: 'WASM' })
    );
  ...
  test('TC-5.5: Focus States', async ({ appPage, page }) => {
    ...
    const isButtonFocused = await appPage.convertButton.evaluate((el) => document.activeElement === el);
