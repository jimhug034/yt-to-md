...
test.describe('Test Suite 7: Format Options (with mocks)', () => {
  ...
  test('TC-7.3: Format Options Panel Visibility', async ({ page }) => {
    ...
    const hasFormatLabel = await page.getByText(/format/i).isVisible().catch(() => false);
    const hasCompactLabel = await page.getByText(/compact/i).isVisible().catch(() => false);
    const hasTimestampLabel = await page.getByText(/timestamp/i).isVisible().catch(() => false);
